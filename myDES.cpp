// myDES.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
public:
    ...
    */
    my_DES my_func(void)
    {
        ...
        if (len % 8 != 0)this->plaintext.append(8 - len % 8, ' ');
        if (len > 100) {
            cout << "明文过长" << endl;
            return *this;
        }
        len = this->plaintext.length();
        strcpy_s(arr, this->plaintext.c_str());
        for (int i = 0; i < len/8; i++)
        {
            memcpy(sub, arr+i*8, 8);
            DES_Efun(sub, this->key, ciphercode);
            BitToChar(ciphercode, cipher, 8);
            this->ciphertext.append(cipher);
        }
        return *this;
    }
    ...
    */
    void my_Dfun(void)
    {
        ...
        if (len % 8 != 0)this->ciphertext.append(8 - len % 8, ' ');
        len = this->ciphertext.length();
        strcpy_s(arr, this->ciphertext.c_str());

        for (int i = 0; i < len / 8; i++)
        {
            memcpy(sub, arr + i * 8, 8);
            CharToBit(sub, ciphercode, 8);
            DES_Dfun(ciphercode, this->key, sub);
            plaintext.append(sub);
        }
    ...
    */
    ...
    friend istream& operator>>(istream& iss, my_DES& DES)
    {
        cout << "请输入明文:" << endl;
        iss >> DES.plaintext;
        cout << "请输入密匙:" << endl;
    ...
    *****************************************************
    */
    friend ostream& operator <<(ostream& oss, my_DES& DES)
    {
        oss << "密文是: ";
        oss << DES.ciphertext << endl;
        DES.my_Dfun();
        cout << endl;
        return oss;
    }
